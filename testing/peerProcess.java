...
public class peerProcess {
    ...
    private static reader commonConfig= new reader("testing/Common.cfg");

public static void readPeerInfo() throws FileNotFoundException {
    try {
        File peerFile = new File("testing/PeerInfo.cfg");
    ...
    public void run(){
        try{
            for(Map.Entry<Integer,Peer> entry : peers.entrySet()){
                System.out.println(myID + " " + entry.getKey());
                if (entry.getKey() != myID) {
                    ...
                    try {
                        ...
                        System.out.println("Waiting to connect on port " + peers.get(newPeer.getID()).getPort());
                        System.out.println(newPeer.getHost() + " " +newPeer.getPort());
...
public static class connectFromPeers extends Thread{
    public void run(){
        try{
            ...
            ServerSocket listener = new ServerSocket(peers.get(myID).getPort());
            while(count >1 ){
                System.out.println("Awaiting connection on port " + peers.get(myID).getPort());
                ...
                outputStream.write(hs.getMessage());
                outputStream.flush();
                log.connectionFrom(myID, thatPeerID);
    ...
    public static void main(String[] args) {
        ...
        try{
            ...
            File logFile = new File(System.getProperty("user.dir") + "/" + "log_peer_" + myID + ".log");
            if (!logFile.exists()) {
            ...
            connectToPeers c = new connectToPeers();
            connectFromPeers f = new connectFromPeers();
            c.start();
            f.start();
